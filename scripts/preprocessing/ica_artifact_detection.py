...
def detect_ica_artifacts(datasets=None, verbose=True, show_plots=False, 
                        z_threshold_blink=5.0, z_threshold_saccade=5.0):
    """
    Detect and save ICA artifacts (blinks and saccades) for all subjects in the specified datasets.
    ...
    """
    print("[INFO] Starting ICA artifact detection...")
    
    if datasets is None:
        datasets = DATASETS.copy()
        # Remove datasets that are not ready for artifact detection
        datasets.pop('braboszcz2017', None)
        # Keep jin2019 for artifact detection
    
    total_processed = 0
    total_skipped = 0
    
    for dataset, subject, label, item, kwargs in iterate_dataset_items(datasets):
        print(f"[INFO] Processing ICA artifact detection for Subject: {subject}, {label}-{item}")
    ...
    if len(saccade_inds) != 0:
        ...
        total_processed += 1
        print(f"[INFO] Completed ICA artifact detection for Subject: {subject}, {label}-{item}")

    print(f"[INFO] ICA artifact detection completed. Processed: {total_processed}, Skipped: {total_skipped}")
    return {"processed": total_processed, "skipped": total_skipped}


if __name__ == "__main__":
    # Run the function when script is executed directly
    detect_ica_artifacts(verbose=True, show_plots=False)
