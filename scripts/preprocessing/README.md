...
*   **Process**:
    ...
    3.  **Apply ICA**:
        *   Applies a common average reference to the data, as ICA models were trained on referenced data.
...
*   **Output**:
    ...
    *   Quality control plots (PSDs, evoked responses) saved in `reports/plots/`.
*   **Action Required**:
...
This pipeline aims to maximize signal integrity by avoiding distortive filtering on the final data, relying instead on robust techniques like RANSAC, AutoReject (implicitly via thresholding in `main_prep.py`), and ICA for artifact removal.

A key aspect of this approach to preserving spectral integrity is the use of linear detrending and baseline correction on epochs, rather than traditional high-pass filtering. This strategy effectively removes slow drifts and DC offsets while keeping the low-frequency spectrum intact. This method is effective under the assumption that epoch lengths are sufficiently short to minimize the impact of constant sensor drift within each epoch.
